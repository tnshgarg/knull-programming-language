...
Interpreter.prototype.functionCall = function (currentFunction) {
  ...
  for (let j = 0; j < currentArguments.length; j++) {
    ...
    if (
      ...
      currentArgument =
        otherInterpreter.input(currentArguments[j]) || currentArguments[j];
    }
...
Interpreter.prototype.conditional = function () {
  if (this.getConditionalKeyword() !== "if") {
    ...
    while (!condition) {
      if (!this.isWrapper())
        throw new Error("A conditional statement requires an opening wrapper!");
      this.get();
      this.consumeUntil(":", "array");
      if (!this.isWrapper())
        throw new Error("A conditional statement requires a closing wrapper!");
      this.get();
...
Interpreter.prototype.loop = function () {
  ...
  if (this.peek() === "from") {
    ...
    firstIndex = this.get();
    if (this.peek() !== "to")
      throw new Error('A from loop should always include the "to" keyword!');
    this.get();
    ...
    finalIndex = this.get();
    if (this.peek() !== "with")
      throw new Error('A from loop should always include the "with" keyword!');
    this.get();
    if (!this.isLetter())
      throw new Error(
...
let knull = new Interpreter(memory);

// BASIC VARIABLE DECLARATION
// Running "knull.memory" will show that the newly-declared
